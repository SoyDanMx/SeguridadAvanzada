...
const WHATSAPP_SERVICE_MESSAGE =
  process.env.NEXT_PUBLIC_WHATSAPP_SERVICE_MESSAGE ??
  "Hola, me gustaría solicitar un servicio de videovigilancia o seguridad electrónica.";
      ...
      <div className="relative z-10 w-full px-4 pb-10 pt-20 sm:pb-12 md:pb-14">
        <div className="container mx-auto flex flex-col items-stretch gap-4 sm:items-center sm:flex-row sm:gap-4">
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:flex-wrap sm:gap-4">
            <a
              href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(WHATSAPP_SERVICE_MESSAGE)}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-8 py-4 text-lg font-semibold text-white shadow-lg transition-all duration-300 hover:bg-[#20C35A] hover:shadow-xl active:scale-[0.98] md:w-auto"

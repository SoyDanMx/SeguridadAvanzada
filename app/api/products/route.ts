...
/**
 * Proxy de productos Syscom. Normaliza formato (titulo/modelo/precio objeto) y aplica margen en MXN.
 * Syscom solo acepta categoria con ID numérico; para slugs sin syscomId usamos busqueda (label).
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Syscom solo acepta categoria con ID numérico; slugs como "audio-y-video" causan 422
    const category = categoryRaw ? resolveCategoryParam(categoryRaw) : "22";
    const search =
      userSearch?.trim() ||
      (catBySlug && !catBySlug.syscomId ? catBySlug.label : undefined);

    // Syscom exige al menos uno de: marca, categoria o busqueda
